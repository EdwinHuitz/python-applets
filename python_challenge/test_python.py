...
#! remember to change skip dir from answers to challenges
@parametrize('ans',['hello'])
@skip('sayHello' not in dir(answers),reason="This function has yet to be declared")
def test_zero(ans):
   assert sayHello() == ans

@parametrize('p1,ans',[(-1,0),(2,3),(5,6)])
@skip('addOne' not in dir(answers),reason="This function has yet to be declared")
def test_one(p1,ans):
   assert addOne(p1) == ans

@parametrize('p1,p2,ans',[(1,1,2),(1,2,3),(-10,16,6),('Hello',5,'nan')])
@skip('addTwoNumbers' not in dir(answers),reason="This function has yet to be declared")
def test_two(p1,p2,ans):
   assert addTwoNumbers(p1,p2) == ans

@parametrize('p1,ans',[([10],10),([2,10,-5],7),([5,10],15),([],0)])
@skip('sumNumbers' not in dir(answers),reason="This function has yet to be declared")
def test_three(p1,ans):
   assert sumNumbers(p1) == ans

@parametrize('p1,ans',[((2,7,0),9),((0,7,-1),6)])
@skip('addList' not in dir(answers),reason="This function has yet to be declared")
def test_four(p1,ans):
   assert addList(p1[0],p1[1],p1[2]) == ans
   assert addList() == 0

@parametrize('p1,p2,ans',[(4,0,math.inf),(10,2,0),(10.5,3,1.5)])
@skip('computeRemainder' not in dir(answers),reason="This function has yet to be declared")
...
@skip('ranges' not in dir(answers),reason="This function has yet to be declared")
def test_six(p1,p2,ans):
   assert ranges(p1,p2) == ans

@parametrize('p1,ans',[('SEI Rocks!','!SKCOR IES')])
@skip('reverseUpcaseString' not in dir(answers),reason="This function has yet to be declared")
def test_seven(p1,ans):
   assert reverseUpcaseString(p1) == ans

@parametrize('p1,ans',[('a',''),('abc','b'),('123456789','2345678')])
@skip('removeEnds' not in dir(answers),reason="This function has yet to be declared")
...
@skip('charCount' not in dir(answers),reason="This function has yet to be declared")
def test_nine(p1,ans):
   assert charCount(p1) == ans

@parametrize('p1,p2,p3,ans',[(1234,'*',3,'1234'),(123,'0',5,'00123'),(42,'*',10,'********42')])
@skip('formatWithPadding' not in dir(answers),reason="This function has yet to be declared")
...
@skip('isPalindrome' not in dir(answers),reason="This function has yet to be declared")
def test_eleven(p1,ans):
   assert isPalindrome(p1) == ans

@parametrize('p1,p2,ans',[('abc','abc',0),('a1c','a2c',1),('!!!!','****',4)])
@skip('hammingDistance' not in dir(answers),reason="This function has yet to be declared")
def test_twelve(p1,p2,ans):
   assert hammingDistance(p1,p2) == ans

@parametrize('p1,ans',[('X','X'),('abc','a-bb-ccc'),('121','1-22-111'),('!A 2','!-AA-   -2222')])
@skip('mumble' not in dir(answers),reason="This function has yet to be declared")
...
@skip('fromPairs' not in dir(answers),reason="This function has yet to be declared")
...
@skip('mergeObjects' not in dir(answers),reason="This function has yet to be declared")
...
@skip('findHighestPriced' not in dir(answers),reason="This function has yet to be declared")
